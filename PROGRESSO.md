...
| 99 testes unitários             | ✅     | `UsuarioDAOTest`, `NotaDAOTest`, `ItemArvoreDAOTest`, `TarefaDAOTest`, `EventoDAOTest`, `UsuarioServiceTest`, `TarefaServiceTest`, `HashUtilTest`, `FormatadorDataTest` |
...
| 29/jun/2026  | Faxina de código e expansão de testes: (1) correção de resource leak em `EventoDAO.inserir()` (`ResultSet` sem try-with-resources); (2) `LoginController` refatorado para usar `UsuarioService.buscarPorEmail/redefinirSenha()` em vez de instanciar `UsuarioDAO` diretamente — corrige violação de arquitetura em camada; (3) `TarefaService` e `EventoService` promovidos a campos de instância em `MainController` (eram instanciados a cada chamada de `verificarPrazos()`); (4) import duplicado de `ThemeManager` removido de `LoginController`; import não-usado de `UsuarioDAO` removido de `MainController`; (5) 3 blocos de Javadoc órfãos corrigidos (`DisciplinaDAO`, `SessaoPomodoroDAO`, `PomodoroService`) e Javadoc adicionado a `buscarResumoRecentes()` e `renomearDisciplina()`; (6) `formatarTempo(int)` centralizado em `FormatadorData` — eliminadas 3 cópias locais em `DashboardController`, `PlanoEstudosController` e `PomodoroTimerController`; (7) `PREFS_PATH` no `PomodoroTimerController` corrigido para usar `DatabaseManager.getDirApp()` em vez de hard-coded `~/.studyapp`; (8) 8 chamadas `printStackTrace()` em threads de background substituídas por `catch (Exception ignored)` (`PomodoroTimerController` × 3, `PlanoEstudosController` × 2, `ThemeManager` × 3); (9) 30 novos testes adicionados: `TarefaDAOTest` (11), `EventoDAOTest` (9), `TarefaServiceTest` (10) — total de 99 testes passando. |
| 29/jun/2026  | Correção do divider 30/70 do SplitPane no módulo de arquivos: `setDividerPositions(0.30)` movido para dentro de `handleNavArquivos()` via `Platform.runLater()`, aplicado após o painel tornar-se visível e o JavaFX recalcular o layout. |
